 ...
 */
/**
 * The Google Docs faker — the PHP twin of the js package's `src/faker.ts`.
 *
...
final class GoogleDocsFaker
{
    ...
    public static function respond(string $operation, array $request): mixed
    {
        ...
        return match ($operation) {
            'document_create' => self::DocumentCreate($config, $fake),
    ...
    private static function DocumentCreate(array $config, mixed $fake): array
    {
        ...
        'title' => ((($v = $config['title'] ?? null) !== null && $v !== '') ? (string) $v : 'Untitled document'),
    ];
    }
}
