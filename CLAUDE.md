...
npm run provider -- google-docs            # regenerate this repo
npm run provider -- google-docs --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/google-docs-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/google-docs-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/google-docs-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-google-docs` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/google-docs/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/google-docs` |
| npm | `@particle-academy/google-docs-ui`, `@particle-academy/google-docs-js` |
| Packagist | `particle-academy/google-docs-php` |
| PyPI | `fancy-google-docs` |
...
**Default to plain HTTP.** A Google Docs SDK is third-party code and is subject to
