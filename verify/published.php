...
/*
 * Google Docs — the published Composer package.
 *
...
if (! is_file($autoload)) {
    ...
    fwrite(STDERR, '    composer require particle-academy/google-docs-php'.PHP_EOL);
    exit(2);
}
...
foreach ($goldens as $golden) {
    ...
    $fake = new FakeValues(FakeValues::seedForCall('google_docs', $operation, $config));
    $faked = GoogleDocsFaker::respond($operation, ['config' => $config, 'fake' => $fake]);
