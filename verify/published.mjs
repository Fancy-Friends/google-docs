/*
 * Google Docs — the published npm packages.
 *
...
import { googleDocsFaker } from "@particle-academy/google-docs-js";
import { GOOGLE_DOCS_KINDS } from "@particle-academy/google-docs-ui";
 ...
 */
const resolved = import.meta.resolve("@particle-academy/google-docs-js");
...
for (const { operation, config, expected } of GOLDENS) {
  const faked = googleDocsFaker(operation, fakeRequest("google_docs", operation, config));
...
assert.equal(GOOGLE_DOCS_KINDS.length, 1);
for (const kind of GOOGLE_DOCS_KINDS) {
...
console.log(`  ok   ui kinds resolve from ${"@particle-academy/google-docs-ui"}`);
